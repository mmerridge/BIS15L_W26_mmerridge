...
```{r message=FALSE, warning=FALSE}
life_history <- read_csv("data/mammal_lifehistories_v2.csv", na="-999") %>% #treat -999 as na
  clean_names()
```
...
There is a negative relationship between mass and litter size - larger mammals have less offspring
```{r}
life_history %>% 
...
life_history %>% 
  ...
  labs(title="Mass vs Litter Size", 
       x="Mass (g)", 
       y="Litter Size")+
  theme(plot.title=element_text(size=rel(1.5), hjust=.5)) #size of title relative to x and y axis, and adjusted to middle
```
