...
```
```{r}
p+geom_jitter(size=3) #helps with overplotting
```
...
```{r}
p+geom_point(size=3, color="black", alpha=0.8, shape=24, fill="blue") #color is outline, fill is inside, alpha is transparency
```
...
```{r}
# Plot all available point shapes, 0-14 are outline shapes (no color), 15-20 are fill shapes (black only), 21-24 can be filled with color
...
```{r}
homerange %>% 
  ggplot(aes(x=taxon, fill=trophic_guild))+ #stacked bar plot (dont do eek)
  geom_bar()+
  coord_flip()+
  labs(title="Observations by Taxon",
       x="Taxonomic Group",
...
homerange %>% 
  ...
  geom_bar(position="dodge")+ #side by side bar plot (much better)
  coord_flip()+
  labs(title="Observations by Taxon",
       x="Taxonomic Group",
       y=NULL) 
...
homerange %>% 
  ...
  theme(axis.text.x=element_text(angle=60, hjust=0.85))+ #adjust x axis labels, tilt and adjust where they are
  labs(title="Observations by Taxon",
       x="Taxonomic Group",
       y=NULL) 
```
...
homerange %>% 
  ...
  scale_y_continuous(labels=scales::percent)+ #scales bar to percentage
  coord_flip()
```
...
penguins %>% 
  ...
  geom_bar()+
  labs(title="Observations by Species",x="Island")
```
...
penguins %>% 
  ...
  geom_bar(position="dodge")+
  labs(title="Observations by Sex",x="Island")
```
