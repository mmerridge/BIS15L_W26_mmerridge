...
```{r}
amniota <- read_csv("data/amniota.csv", na = "-999") %>% 
  clean_names()
```
...
Subspecies and female body mass at maturity (g)
```{r}
gg_miss_var(amniota)
```
...
Mammals having 100% missing data for egg mass makes biological sense since they don't lay eggs. Birds and reptiles having 50% and 92% missing data is a bit more concerning, and means that it was probably harder to get the measurments, since they do lay eggs. This means that you can't treat all NA's the same, since they also share information about the data.
```{r}
amniota %>%
  group_by(class) %>%
  miss_var_summary() %>%
  filter(variable == "egg_mass_g")
```
